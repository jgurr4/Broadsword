 ...
 * interior whose STAIRS tile leads back out; {@code enemy} is the single
 * optional Octorock inside (rock caves only — the secret cave is empty).
 *
 * <p>Cave state is non-persistent: the interior is re-derived from the seed,
 * only "Link is inside cave X" goes into a save.
 */
public record Cave(ScreenPos entry, Screen room, int entryTx, int entryTy, EnemySpawn enemy) {
